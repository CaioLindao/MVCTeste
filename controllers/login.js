...
const admin_login = async (req, res) => {
    ...
    if (!(login.length && password.length)) {
        io.emit("badLogin", { message: "Informe Login e Senha"})
        res.sendStatus(422);
    } else {
            ...
            find.forEach(async admin => {
                ...
                if (comparison) {
                    if (login == admin.login) {
                        ...
                        res.sendStatus(200);
                        io.emit("login", { message: "Entrando...", code: 100})
                    }
                }
            });
            // Caso nenhum corresponda, retorna o resultado ao client
            io.emit("badLogin", { message: "Login ou Senha incorretos."});
        } catch (error) {
            console.log(error)
            io.emit("badLogin", { message: "Erro de requisição."})
        }
    }
}
